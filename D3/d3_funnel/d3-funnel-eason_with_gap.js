	  ...
	  */
	   ...
	   */
			...
			value: function _initialize(data, options) {
				...
				//eason
                this.strokeWidth = settings.chart.strokeWidth;
				// Support for events
	   ...
	   */
			...
			value: function _draw() {
				// Add the SVG
				this.svg = _d2.default.select(this.selector).append('svg').attr('width', this.width).attr('height', this.height).attr('style', 'padding: 10px');
	   ...
	   */
			...
			value: function _drawBlock(index) {
				...
				if (this.animation !== 0) {
					path.transition().duration(this.animation).ease('linear').attr('fill', this.blocks[index].fill.actual)
						.attr('d', this._getPathDefinition(index))
						.attr('stroke','#ffffff')
						.attr('stroke-width', this.strokeWidth || 0)
						.each('end', function () {
	   ...
	   */
			...
			value: function _addBlockLabel(group, index) {
				...
				var percent =  this.blocks[index].label.percent;
				...
				var y1 = this._getTextYSpecific(paths);
				group.append('text').text(percent).attr({
					x: x,
					y: y1,
					fill: '#000',
					'text-anchor': 'middle',
	   ...
	   */
			...
			key: '_getTextYSpecific',
			value: function _getTextYSpecific(paths) {
				return paths[1][1] + 3 ;
			}
		}]);
